...
document.querySelector('#btn').addEventListener('click',function(){
    ...
    // 지뢰의 위치를 랜덤하게 뽑아준다.
    var 후보군 = Array(hor*ver).fill().map(function (element,index){return index})
    var 셔플 =[];
        ...
        for (var j=0;j<hor;j++){
            ...
            td.addEventListener('contextmenu',function (e){
                ...
                if (e.currentTarget.textContent==='' || e.currentTarget.textContent==='X'){
                    ...
                    e.currentTarget.classList.toggle('flag');
                    if (dataset[줄][칸]=== 코드표.지뢰) dataset[줄][칸]=코드표.깃발지뢰;
                    else dataset[줄][칸]= 코드표.깃발;
                }else if (e.currentTarget.textContent==='!'){
                    e.currentTarget.textContent='?';
                    e.currentTarget.classList.toggle('flag');
                    e.currentTarget.classList.toggle('question');
                    if (dataset[줄][칸]=== 코드표.깃발지뢰) dataset[줄][칸]=코드표.물음표지뢰;
                    else dataset[줄][칸]= 코드표.물음표;
                }else if (e.currentTarget.textContent==='?'){
                    e.currentTarget.classList.toggle('question');
            ...
            td.addEventListener('click',function(e){
                ...
                if (dataset[줄][칸]===코드표.연칸 || dataset[줄][칸]===코드표.물음표지뢰|| dataset[줄][칸]===코드표.깃발지뢰 || dataset[줄][칸]===코드표.물음표||
                dataset[줄][칸]===코드표.깃발) return;
                ...
                if (dataset[줄][칸]===코드표.지뢰){ // 클릭했을때 지뢰일때
                    e.currentTarget.textContent='펑';
                    document.querySelector('#result').textContent='실패';
                    중단플래그 = true; // 게임 종료
                }else { // 클릭했을때 지뢰 아닐 때, 주변의 지뢰 개수를 카운트
                    dataset[줄][칸]=코드표.연칸; // 열은 칸을 1로 변경
                    ...
                    var 주변지뢰개수 =주변지뢰.filter(function(element){
                        return [코드표.지뢰,코드표.물음표지뢰,코드표.깃발지뢰].includes(element)
                    }).length;
    ...
    for (var x=0;x<셔플.length;x++){
        var 세로= Math.floor(셔플[x] / ver);
        var 가로= (셔플[x] % ver);
        tbody.children[세로].children[가로].textContent= 'X'; // 화면
        dataset[세로][가로]=코드표.지뢰; // 데이타셋
    }
